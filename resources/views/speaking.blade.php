...
@section('title', 'Latihan Percakapan Mandarin')

@section('content')
  <div class="latihan-mandarin">
    ...
    <div class="controls">
        ...
        <div id="paginationControls">
            ...
            <span id="pageInfo">Halaman 1/2</span>
            <button id="nextPageBtn" onclick="changePage(currentPage + 1)">Selanjutnya →</button>
        </div>
    </div>

    <div class="dialog-container">
        <h3>Dialog Aktif: <span id="dialogTitle">Sapaan & Perkenalan 👋</span></h3>
...
@section('js')
    <script>
        ...
        // --- Data Dialog (20 Set) ---
        // Kita ubah struktur data agar mudah dipaginasi.
        const allTopics = [
        ...
        let currentPage = 1;
        // PENTING: Hitung ulang total halaman berdasarkan allTopics yang baru
        const totalPages = Math.ceil(allTopics.length / TOPICS_PER_PAGE); 

        // --- Variabel State Dialog ---
        let currentDialogue = allTopics[0].lines; 
        let activeTarget = allTopics[0];
        ...
        /**
         * @description Mengambil daftar suara TTS di latar belakang dan mengontrol status tombol.
         */
        function initVoices() {
            if (!('speechSynthesis' in window)) {
                updateStatus("Browser tidak mendukung Speech Synthesis. Fitur Dengarkan tidak tersedia.", 'red');
                listenBtn.textContent = "Fitur Tidak Tersedia";
                listenBtn.style.backgroundColor = 'gray';
                listenBtn.onclick = () => alert("Browser Anda tidak mendukung Text-to-Speech.");
                return;
            }

            listenBtn.textContent = "Memuat Suara... 🎧";
            ...
            const attemptLoadVoices = () => {
                ...
                if (voices.length === 0) {
                    setTimeout(attemptLoadVoices, 0); 
                    return;
                }
                
                selectedVoice = voices.find(voice => voice.lang.startsWith('zh-CN') && (voice.name.includes('Google') || voice.name.includes('Tingting') || voice.name.includes('Microsoft'))) || 
                               voices.find(voice => voice.lang.startsWith('zh-'));
                
                // Finalisasi status suara
                listenBtn.classList.remove('loading');
                listenBtn.textContent = "Dengarkan Baris Aktif 🎧";
                listenBtn.style.backgroundColor = '#1e3a8a';

                if (!selectedVoice) {
                    updateStatus("Peringatan: Suara Mandarin yang alami tidak ditemukan. Menggunakan suara default sistem.", 'red');
                } else {
                    updateStatus(`Klik baris dialog untuk mendengarkan.`, '#1e3a8a');
                }
            };
        ...
        // --- Paginasi & Render Dropdown ---
        function renderTopicSelector(page) {
            dialogSelector.innerHTML = '';
            
            const start = (page - 1) * TOPICS_PER_PAGE;
            const end = start + TOPICS_PER_PAGE;
            const topicsOnPage = allTopics.slice(start, end);
            
            // Mengelompokkan ulang topik berdasarkan 'group'
            const groupedTopics = topicsOnPage.reduce((acc, topic) => {
                if (!acc[topic.group]) {
                    acc[topic.group] = [];
                }
            ...
            for (const groupName in groupedTopics) {
                const optgroup = document.createElement('optgroup');
                optgroup.label = groupName;
                
                groupedTopics[groupName].forEach((topic, index) => {
                    ...
                    if (!firstKey) {
                        firstKey = topic.key; // Simpan kunci topik pertama
                    }
                });
            ...
            // Otomatis muat dialog pertama di halaman baru
            if (firstKey) {
                loadDialogue(firstKey, true); // true untuk jangan ulangi set active target
            }
        }
        ...
        function changePage(newPage) {
            if (newPage >= 1 && newPage <= totalPages) {
                currentPage = newPage;
                renderTopicSelector(currentPage);
            }
        }

        // --- Load Dialog dari Dropdown ---
        function loadDialogue(key, isInitialLoad = false) {
            window.speechSynthesis.cancel();
            
            const dialogData = allTopics.find(t => t.key === key);
            if (!dialogData) return;

            currentDialogue = dialogData.lines; 
            activeTarget = null;
            
            document.getElementById('dialogTitle').textContent = dialogData.title;
            ...
            currentDialogue.forEach((line, index) => {
                ...
                `;
                div.onclick = () => setActiveTarget(index);
                dialogDisplay.appendChild(div);
            });
            
            // Set baris pertama sebagai aktif secara default
            if (!isInitialLoad) {
                setActiveTarget(0); 
            } else {
                // Saat render selector, kita hanya memuatnya tanpa set aktif (untuk mencegah speech call)
                document.querySelector('.dialog-line[data-index="0"]').classList.add('active');
                activeTarget = currentDialogue[0];
                updateStatus("Dialog baru dimuat. Klik salah satu baris untuk mendengarkan pelafalan.");
            }
        }
        ...
        function setActiveTarget(index) {
            window.speechSynthesis.cancel();
            document.querySelectorAll('.dialog-line').forEach(el => el.classList.remove('active'));

            const selectedLine = document.querySelector(`.dialog-line[data-index='${index}']`);
            if (selectedLine) {
                selectedLine.classList.add('active');
                activeTarget = currentDialogue[index];
                
                updateStatus(`Siap mendengarkan: "${activeTarget.hanzi}". Tekan tombol Dengarkan 🎧.`);
            }
        }
        
        // --- Text-to-Speech (Dengarkan) ---
        function readActiveTarget() { 
            if (!activeTarget) {
                updateStatus("Pilih baris dialog yang ingin Anda dengarkan terlebih dahulu.", 'orange');
                return;
            }

            if ('speechSynthesis' in window) {
                if (!selectedVoice) {
                    updateStatus("🚨 Suara TTS belum siap. Harap tunggu sebentar atau klik lagi. Proses memuat suara sedang berlangsung.", 'red');
                    initVoices(); 
                    return;
                }
                ...
                const utterance = new SpeechSynthesisUtterance(activeTarget.hanzi);
                utterance.lang = 'zh-CN'; 
                utterance.rate = 0.9; 
                utterance.voice = selectedVoice;
                
                utterance.onstart = () => {
                    listenBtn.textContent = "🔊 Sedang Memutar...";
                    updateStatus("🎧 Sedang memutar audio...", 'orange');
                }
                utterance.onend = () => {
                    listenBtn.textContent = "Dengarkan Baris Aktif 🎧";
                    updateStatus(`Pemutaran selesai. Baris aktif: "${activeTarget.hanzi}"`);
                }
        ...
        // INIT UTAMA
        window.onload = () => {
            renderTopicSelector(currentPage); // Tampilkan hanya 10 topik pertama
            initVoices();
        };
        
        // Bersihkan speech synthesis saat menutup
